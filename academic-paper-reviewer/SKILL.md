...
**Mode A — orchestrator-driven (default):** the pipeline orchestrator (in `academic-pipeline` skill) dispatches `academic-paper-reviewer` as part of the full ARS pipeline Stage 3 (Review).
...
Routing into Mode B requires explicit user signal — say "use phase-by-phase mode" or prefix with `[direct-mode]`. Hermes will clarify ambiguous cross-phase input per `../shared/references/intent_clarification_protocol.md`.
...
Use `skill_view(name="academic-paper-reviewer", file_path="references/...")` to load any reference file.

**Key Hermes equivalents:**
| Claude Code | Hermes |
|-------------|--------|
| Agent dispatch | `delegate_task(goal=..., context=...)` |
| `/ars-<mode>` | `/skill academic-paper-reviewer` or natural language |
