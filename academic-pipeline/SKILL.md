...
> **Routing discipline (v3.9.2):** see `../shared/references/intent_clarification_protocol.md` for cross-skill routing rules. Hermes auto-detects the appropriate skill based on trigger keywords; you can also use `/skill <name>` to explicitly load a skill.
...
| # | Agent | Role | File |
|---|-------|------|------|
| 1 | the pipeline orchestrator | Main orchestrator: detects stage, recommends mode, triggers skill, manages transitions | `agents/pipeline_orchestrator_agent.md` |
...
- the pipeline orchestrator (D — orchestrator, full pipeline visibility)
...
This skill was adapted from [academic-research-skills](https://github.com/Imbad0202/academic-research-skills) (originally for Claude Code) to work with Hermes Agent.
...
```
Or Hermes will auto-detect based on trigger keywords in your message.

**Subagent orchestration:**  
The multi-agent pipeline uses Hermes' `delegate_task` for parallel execution. Each agent in the Agent Team is dispatched as a subagent with its definition file (`agents/*.md`) as the goal context. Parallel phases (e.g., reviewer panel) use `delegate_task(tasks=[...])` for concurrent execution.

**View linked references:**
Use `skill_view(name="academic-pipeline", file_path="references/...")` to load any reference file.

**Key Hermes equivalents:**
| Claude Code | Hermes |
|-------------|--------|
| Agent dispatch | `delegate_task(goal=..., context=...)` |
| `/ars-<mode>` | `/skill academic-pipeline` or natural language |
| `pipeline_orchestrator_agent` | Orchestrator via sequential `delegate_task` calls |
| Claude Code subagents | Hermes `delegate_task` subagents |
| File reads | `read_file` / `skill_view(name, file_path)` |
| Web search | `web_search` tool |
| API calls | `terminal` with `curl` or `execute_code` with Python |

**Important:** All IRON RULES, quality standards, anti-patterns, and checkpoint rules from the original skill are preserved exactly as designed. The adaptation only changes the execution layer (Claude Code → Hermes), not the research methodology.
