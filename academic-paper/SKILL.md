...
**Mode A — orchestrator-driven (default):** the orchestrator (in `academic-pipeline` skill) uses `delegate_task` to run each phase sequentially with state tracking.
...
Use `skill_view(name="academic-paper", file_path="references/...")` to load any reference file.

**Key Hermes equivalents:**
| Claude Code | Hermes |
|-------------|--------|
| Agent dispatch | `delegate_task(goal=..., context=...)` |
| `/ars-<mode>` | `/skill academic-paper` or natural language |
