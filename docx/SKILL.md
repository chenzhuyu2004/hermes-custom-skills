---
name: docx
description: "Create, read, edit, or manipulate Word documents (.docx files). Triggers: Word doc, .docx, report, memo, letter, template, tracked changes, comments, find-and-replace, document formatting, tables of contents, headings, page numbers. Do NOT use for PDFs, spreadsheets, or Google Docs."
version: "3.0.0"
author: "Hermes Agent (adapted from Anthropic official docx skill)"
license: MIT
metadata:
  hermes:
    tags: [docx, word, documents, office, writing, formatting]
...
Generate .docx files with JavaScript (`docx` npm package). Install: `npm install -g docx`
...
Edit files in `unpacked/word/`. See [XML Reference](#xml-reference) below for patterns.

**Use "Claude" as the author** for tracked changes and comments, unless the user explicitly requests use of a different name.

**Use the `patch` tool directly for string replacement. Do not write Python scripts.** Scripts introduce unnecessary complexity. The `patch` tool shows exactly what is being replaced.
...
# Add new paragraph with style
p = doc.add_paragraph('New content', style='Normal')
run = p.runs[0]
run.font.name = 'Times New Roman'
run.font.size = Pt(12)

# Add table (simpler API than docx-js)
...
Current setup:
- `npm` for docx-js: `C:\Program Files\nodejs\npm.cmd`
- `pandoc`: installed via winget
- `python`=3.11.x: python-docx available
- LibreOffice: optional, install via winget if needed for .doc conversion
